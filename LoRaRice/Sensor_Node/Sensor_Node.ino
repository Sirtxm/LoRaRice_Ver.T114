...
void downLinkDataHandle(McpsIndication_t *mcpsIndication) {
  if (mcpsIndication->BufferSize >= 2) {
    ...
    Serial.printf("⏱️ Received sleep time from TTN: %lu ms\n", receivedSleepDuration);
  }
}
...
void enterDeepSleep() {
  ...
  delay(100);  // ให้ Serial ส่งข้อความออกให้ทัน
  sd_power_system_off();  // System OFF: MCU หยุดหมด, ตื่นด้วย RTC
}
...
void setup() {
  ...
  LoRaWAN.join();
  LoRaWAN.setDownLinkDataHandle(downLinkDataHandle);
}
